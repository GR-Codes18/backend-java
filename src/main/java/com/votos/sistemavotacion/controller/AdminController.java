...
import com.votos.sistemavotacion.dto.AdministradorResponse;
...
public class AdminController {
    ...
    @GetMapping("/{id}")
    public ResponseEntity<AdministradorResponse> obtenerPerfil(@PathVariable Integer id) { // Cambia el tipo de retorno
        return administradorRepository.findById(id)
                .map(admin -> {
                    // Mapeo manual de la entidad al DTO, excluyendo la contraseña
                    AdministradorResponse responseDto = AdministradorResponse.builder()
                            .idAdmin(admin.getIdAdmin())
                            .nombre(admin.getNombre())
                            .email(admin.getEmail())
                            .rol(admin.getRol())
                            .regionAcceso(admin.getRegionAcceso())
                            .build();
